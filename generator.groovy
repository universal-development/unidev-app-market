...
new File(FILES_DIR).listFiles().each {
    ...
    app.lastModified = it.lastModified()
...
for(Object app : apps.values()) {
    ...
    links.add(
            loc :  market.rootUrl + '/' + app.outputFolder,
            lastMod:  new java.util.Date(app.lastModified),
            changeFreq: "daily",
            priority: 0.5f
    );
...
for(Object category : categories) {
    ...
    links.add(
            loc : market.rootUrl + '/' +category.link,
            lastMod: new java.util.Date(),
            changeFreq: "daily",
            priority: 0.5f
    );
